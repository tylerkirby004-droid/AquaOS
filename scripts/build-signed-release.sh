...
for artifact in "$binary" "$output_dir/aquaosctl-linux-amd64" "$output_dir/aquaos-admin-linux-amd64" "$output_dir/aquaos-ha-config-linux-amd64"; do
  ...
  digest_input="$artifact.digest-input"
  printf '%s' "$digest" > "$digest_input"
  openssl pkeyutl -sign -rawin -inkey "$AQUAOS_SIGNING_KEY" -in "$digest_input" -out "$artifact.sig"
  rm -f "$digest_input"
  od -An -vtx1 "$artifact.sig" | tr -d ' \n' > "$artifact.sig.hex"
done
cp "$AQUAOS_PUBLIC_KEY_HEX_FILE" "$output_dir/aquaos-ed25519-public-key.hex"
cp configs/aquaos.yaml "$output_dir/aquaos.yaml"
mkdir -p "$output_dir/configs" "$output_dir/infrastructure" "$output_dir/homeassistant" "$output_dir/scripts"
cp configs/aquaos-appliance.yaml "$output_dir/configs/aquaos-appliance.yaml"
mkdir -p "$output_dir/infrastructure/docker"
