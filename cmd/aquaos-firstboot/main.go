...
type installer struct{ script, payload, version, checksum, complete string }

func (i installer) Install(ctx context.Context, request firstboot.InstallRequest) error {
	if _, err := os.Stat(i.complete); err == nil {
		return errors.New("appliance installation is already complete")
	} else if !errors.Is(err, os.ErrNotExist) {
		return fmt.Errorf("inspect completion marker: %w", err)
	}
	args := []string{"--release", i.payload, "--repository", i.payload, "--version", i.version, "--sha256", i.checksum, "--site-id", request.SiteID, "--address", request.Address, "--timezone", request.Timezone, "--apply", "--ack-dedicated-appliance", "--ack-independent-safeguards"}
	...
	if err := command.Run(); err != nil {
		return fmt.Errorf("run signed installer: %w", err)
	}
